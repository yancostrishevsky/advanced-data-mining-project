"""Contains the implementation of the Kolmogorov-Arnold Neural Network (KAN).

This module was copied in its original form from:
...
class KAN(nn.Module):
    def __init__(self, layers_hidden, grid_size=5, spline_order=3, base_activation=nn.GELU, grid_range=[-1, 1]):
        ...
        # Parameters for the linear transformations in each layer.
        self.base_weights = nn.ParameterList()
        # Parameters for the spline-based transformations in each layer.
        self.spline_weights = nn.ParameterList()
        # Layer normalization for each layer to ensure stable training.
        self.layer_norms = nn.ModuleList()
        # PReLU activations for each layer to introduce non-linearity.
        ...
        for i, (in_features, out_features) in enumerate(zip(layers_hidden, layers_hidden[1:])):
            ...
            self.spline_weights.append(nn.Parameter(torch.randn(
    ...
    def forward(self, x):
        ...
        for i, (base_weight, spline_weight, layer_norm, prelu) in enumerate(zip(self.base_weights, self.spline_weights, self.layer_norms, self.prelus)):
            ...
            for k in range(1, self.spline_order + 1):
                ...
                delta = torch.where(right_intervals == left_intervals, torch.ones_like(
                    right_intervals), right_intervals - left_intervals)
            ...
            spline_output = F.linear(bases.view(x.size(0), -1),
                                     spline_weight.view(spline_weight.size(0), -1))
